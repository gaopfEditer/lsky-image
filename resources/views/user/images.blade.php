    ...
    <div class="relative flex justify-between items-center px-2 py-2 z-[3] top-0 left-0 right-0 bg-white border-solid border-b">
        <div class="space-x-2 flex justify-between items-center">
            ...
            <div class="flex-row hidden lg:flex">
                <a data-operate="select-all" class="text-sm py-2 px-3 hover:bg-gray-100 rounded text-gray-800" href="javascript:void(0)">
                    <i class="fas fa-square text-blue-500"></i> 全选
                </a>
            ...
            <div class="block lg:hidden">
                <x-dropdown direction="right">
                    ...
                    <x-slot name="content">
                <x-dropdown-link data-operate="select-all" href="javascript:void(0)" @click="open = false">
                    <i class="fas fa-square text-blue-500 mr-2"></i>全选
                </x-dropdown-link>
    ...
    @push('scripts')
        ...
        <script>
            ...
            const bindOperates = () => {
                ...
                $('[data-operate]').hide();
                let operates = ['select-all']; // 全选按钮一直显示
                if (selected.length === 0) {
                    operates.push('refresh');
                }
                if (selected.length === 1) {
                    operates.push('refresh', 'movements', 'permission', 'detail', 'rename', 'delete');
                }
                if (selected.length > 1) {
                    operates.push('refresh', 'movements', 'permission', 'delete');
                }
                ...
                // 更新全选按钮状态
                let allImageSelectors = $(IMAGES_GRID).find('.image-selector');
        ...
        <script>
            ...
            function selectAllImages() {
                ...
                // 如果当前选中的数量等于总数量，则取消全选
                if (selected.length === allImageSelectors.length) {
                    ds.clearSelection();
                } else {
                    // 全选所有图片 - 通过模拟点击所有image-selector来实现
                    allImageSelectors.each(function() {
                        let $imageItem = $(this).closest('a');
                        // 检查元素是否已被选中
                        let isSelected = selected.some(item => item === $imageItem[0]);
                        if (!isSelected) {
                            ds.toggleSelection($imageItem);
                        }
                    });
                }
                // 更新操作栏状态
