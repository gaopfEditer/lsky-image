<div class="pb-6 h-full">
    ...
    <div class="mb-4 p-4 bg-white rounded-md shadow-custom">
        ...
        <div class="mt-3 rounded-md border-2 border-dotted border-stone-300 w-full h-full" id="picker-dnd" onclick="$('#picker').click()">
            ...
            <div id="upload-preview" class="m-2 hidden relative">
                <div id="upload-all-btn" class="absolute top-2 right-2 z-10 hidden">
                    <button class="px-3 py-1 bg-blue-500 text-white text-sm rounded-md hover:bg-blue-600 transition-colors">
                        <i class="fas fa-upload mr-1"></i>全部上传
                    </button>
                </div>
                <div class="pt-8">
                    <!-- 文件列表将在这里动态插入 -->
...
@push('scripts')
    ...
    <script>
        ...
        let $previews = $('#upload-preview .pt-8');
        ...
        $picker.fileupload({
            ...
            add: (e, data) => { // Return true to continue adding, otherwise terminate
                ...
                let push = function (blob) {
                    ...
                    data.$preview = $previews.append(html).find(`[data-id="${guid}"]`);
                    queue[guid] = data;
        ...
        $previews.on('click', '[data-operate]', function (e) {
            ...
            if (method === 'remove') {
                ...
                // 检查是否还有文件，如果没有就隐藏全部上传按钮
                let hasFiles = false;
                for (let fileId in queue) {
                    if (queue[fileId]) {
                        hasFiles = true;
                        break;
                    }
        ...
        // 全部上传按钮点击事件 - 调用批量上传API
        $('#upload-all-btn').click(function (e) {
            e.stopPropagation();
            let waitingFiles = [];
            for (let id in queue) {
                if (queue[id].status === UPLOAD_WAITING) {
                    waitingFiles.push(queue[id].files[0]);
                }
            }

            if (waitingFiles.length === 0) {
                toastr.info('没有等待上传的文件');
                return;
            }

            // 禁用按钮，显示加载状态
            $(this).prop('disabled', true).html('<i class="fas fa-spinner fa-spin mr-1"></i>上传中...');

            // 准备FormData
            let formData = new FormData();
            waitingFiles.forEach((file, index) => {
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.status) {
                        // 批量上传成功
                        toastr.success(`批量上传成功！成功: ${response.data.success_count}，失败: ${response.data.error_count}`);

                        // 更新队列状态
                        console.log('开始更新队列状态', {
                            queueCount: Object.keys(queue).length,
                            successFiles: response.data.success_files.length,
                            errorFiles: response.data.error_files.length
                        });
                        ...
                        // 先处理成功的文件
                        for (let id in queue) {
                            if (queue[id].status === UPLOAD_WAITING && successIndex < response.data.success_files.length) {
                                console.log('更新成功文件状态', id, successIndex);
                                // 成功上传
                        ...
                        // 再处理失败的文件
                        for (let id in queue) {
                            if (queue[id].status === UPLOAD_WAITING && errorIndex < response.data.error_files.length) {
                                console.log('更新失败文件状态', id, errorIndex);
                                // 上传失败
                                setStatus(queue[id], UPLOAD_ERROR, response.data.error_files[errorIndex].error);
                                errorIndex++;
                            }
                        }

                        console.log('队列状态更新完成', {
                            successIndex,
                            errorIndex,
                            remainingWaiting: Object.values(queue).filter(item => item.status === UPLOAD_WAITING).length
                        });

                        // 显示链接区域
                        $links.show();
                        utils.setCapacityProgress(response.data.success_files.reduce((total, file) => total + file.size, 0) / 1024);

                        // 不要立即删除队列中的文件，让用户可以看到上传结果
                        // 用户可以手动删除或继续上传其他文件
                    } else {
                        toastr.error('批量上传失败: ' + response.message);
                    }
                },
                error: function(xhr) {
                    let errorMsg = '批量上传失败';
                ...
                complete: function() {
                    // 恢复按钮状态
                    $('#upload-all-btn').prop('disabled', false).html('<i class="fas fa-upload mr-1"></i>全部上传');
                }
            });
        });
